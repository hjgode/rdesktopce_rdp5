...
#ifdef DEBUG
	DEBUGMSG(1, (L"binToHexW szHex: %s\n", szHex));
	DEBUGMSG(1, (L"binToHexW pByte: "));
	for(UINT k=0; k<dwLen/2; k++){
		DEBUGMSG(1, (L"%02x", pByte[k]));
	}
	DEBUGMSG(1, (L"\n"));
#endif
	return 0;
...
DWORD hex2bin(TCHAR* szEncodedPwd, BYTE* pByte, DWORD* pSize){
	...
	DEBUGMSG(1, (L"wcslen=%i, strlen=%i\n", wcslen(szEncodedPwd), strlen(szEncodedPwdA)));
